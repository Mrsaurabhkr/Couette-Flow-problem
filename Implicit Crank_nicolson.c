...
int main() 
{
    ...
    double Nj = -Vis/2.0, Pj = (1 + Vis), Sj = -Vis/2.0;
    qi[nodes - 1] = 1.0;

    FILE *file1;
    file1 = fopen("Error_crank_nicolson.txt", "w");
    fprintf(file1, "%.6f\t%.6f\n", Tcurr, log10(Err));
        ...
        do
          {
            ...
            for (int i = 1; i < (nodes - 1); i++) 
             {
                di[i] = (Vis / 2.0) * New_V[i + 1] + (1 - Vis) * New_V[i] + (Vis / 2.0) * New_V[i - 1];
                pi[0] = -Nj / Pj;
                qi[0] = di[i] / Pj;
    ...
    FILE *file2;
    file2 = fopen("velocity_crank_nicolson.plt", "w");
    fprintf(file2, "VARIABLES = \"u\", \"Y\"\n");
    fprintf(file2, "ZONE T = \"CN\"\n\n");
